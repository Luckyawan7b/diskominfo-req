<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        ...
        <div class="flex gap-3">
            <a href="{{ route('konteks.form', $konteks) }}"
                class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-slate-600 text-sm text-slate-300 hover:bg-slate-700 transition-colors">
    ...
    @endif    <p class="text-sm text-slate-400 mb-5 max-w-2xl">
        Satu <span class="text-white font-medium">Sasaran Pembangunan Nasional</span> mendukung pencapaian target kinerja
        <span class="text-white font-medium">Sasaran UPR</span>, dan satu Sasaran UPR dapat memiliki lebih dari satu
    ...
    <div class="space-y-6">
            ...
            <div class="rounded-xl border border-slate-700/50 bg-slate-800/50 p-6" wire:key="block-{{ $block['id'] }}">
                ...
                <div class="flex items-center justify-between gap-2 mb-5 pb-3 border-b border-slate-700/50">
                    <div class="flex items-center gap-2.5">
                        <span
                            class="flex items-center justify-center w-6 h-6 rounded-full bg-emerald-500/20 text-emerald-400 text-xs font-bold shrink-0">{{ $i + 1 }}</span>
                        <span class="text-sm font-bold text-white tracking-wide">Sasaran UPR / Dinas
                            Ke-{{ $i + 1 }}</span>
                    </div>
                    @if ($isEditable)
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <x-textarea-auto wire:model="blocks.{{ $i }}.sasaran_upr" rows="4"
                            :disabled="!$isEditable"
                            placeholder="Contoh: Meningkatnya kecepatan dan kemudahan warga dalam pengurusan surat pelayanan administrasi dinas secara online." />
                    </div>
                    ...
                    <div>
                        ...
                        <x-textarea-auto wire:model="blocks.{{ $i }}.sasaran_nasional" rows="4"
                            :disabled="!$isEditable"
                            placeholder="Contoh: Terwujudnya transformasi digital pelayanan publik dan tata kelola dinas yang akuntabel." />
                    </div>
                </div>
                ...
                <div class="border-t border-slate-700/50 pt-5">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            <p class="text-xs font-semibold text-slate-300 uppercase tracking-wider">3. Indikator &amp;
                                Target Kinerja</p>
                            <p class="text-xs text-slate-400">Pengukuran deskripsi capaian dan ukuran target indikator
                                kinerja.</p>
                        </div>
                    </div>
                        ...
                        @foreach ($block['indikator'] as $j => $ind)
                            <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-start"
                                ...
                                <div class="sm:col-span-7">
                                    <label class="block text-[11px] font-medium text-slate-300 mb-0.5">Indikator
                                        Kinerja</label>
                                    <p class="text-[10px] text-slate-400 mb-1">Diisi dengan indikator kinerja yang
                                ...
                                <div class="sm:col-span-4">
                                    <label class="block text-[11px] font-medium text-slate-300 mb-0.5">Target
                                        Kinerja</label>
                                    <p class="text-[10px] text-slate-400 mb-1">Diisi dengan target kinerja yang
                        ...
                        @if ($isEditable)
                            <div class="pt-2">
                                <button type="button" wire:click="addIndikator({{ $i }})"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-dashed border-emerald-500/40 bg-emerald-500/5 text-xs text-emerald-400 hover:bg-emerald-500/15 transition-colors cursor-pointer">
        ...
        @empty
            <div
                class="rounded-xl border border-dashed border-slate-700/50 px-4 py-12 text-center text-slate-400 text-sm bg-slate-800/20">
                <p class="font-medium text-slate-300 mb-1">Belum ada Sasaran UPR yang ditambahkan.</p>
                <p class="text-xs text-slate-500">Klik tombol di bawah untuk membuat sasaran dinas pertama.</p>
            </div>
        @endforelse
    </div>
    ...
    @if ($isEditable)
        <button wire:click="addBlock"
            class="mt-5 inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-dashed border-slate-600 text-sm text-slate-400 hover:text-emerald-400 hover:border-emerald-500/50 transition-colors cursor-pointer">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
