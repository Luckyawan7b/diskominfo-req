<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-white">Kelola Data Dinas</h1>
            <p class="text-sm text-slate-400 mt-1">Master data wilayah dinas yang terdaftar dalam sistem SPBE</p>
        </div>
    ...
    <div class="mb-4 max-w-sm">
        <input wire:model.live.debounce.300ms="search" type="text"
            ...
            placeholder="Cari dinas atau kecamatan...">
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-700/50">
                    ...
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 uppercase">Nama Dinas</th>
            ...
            <tbody class="divide-y divide-slate-700/30">
                @forelse($dinasList as $dinas)
                    <tr class="hover:bg-slate-700/20 transition-colors">
                        <td class="px-5 py-4 font-mono font-bold text-emerald-400">{{ $dinas->alias }}</td>
                        <td class="px-5 py-4 text-white font-medium">{{ $dinas->nama_dinas }}</td>
                        <td class="px-5 py-4 text-slate-400 text-xs">
                            {{ $dinas->kecamatan ?: '-' }}, {{ $dinas->kabupaten ?: '-' }}
                        </td>
                        <td class="px-5 py-4 text-center text-slate-300">{{ $dinas->users_count }}</td>
                        <td class="px-5 py-4 text-center text-slate-300">{{ $dinas->konteks_count }}</td>
                        <td class="px-5 py-4 text-right space-x-2">
                            <button wire:click="openEditModal({{ $dinas->id }})" class="text-emerald-400 hover:text-emerald-300 text-xs font-medium cursor-pointer">
                                Edit
                            </button>
                            <button wire:click="deleteDesa({{ $dinas->id }})" wire:confirm="Hapus dinas ini beserta datanya?" class="text-red-400 hover:text-red-300 text-xs font-medium cursor-pointer">
                                Hapus
                            </button>
                        </td>
                    </tr>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-slate-800 border border-slate-700/50 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
                <h3 class="text-base font-semibold text-white">
                    {{ $editingId ? 'Edit Data Dinas' : 'Tambah Dinas Baru' }}
                </h3>

                <form wire:submit="save" class="space-y-4">
                    <div>
                        <label class="block text-xs font-medium text-slate-300 mb-1">Kode Dinas <span class="text-red-400">*</span></label>
                        <input wire:model="alias" type="text" class="w-full rounded-lg border border-slate-600 bg-slate-700/50 px-3 py-2 text-white text-sm uppercase focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="SKM">
                        @error('alias') <p class="mt-1 text-xs text-red-400">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-slate-300 mb-1">Nama Dinas <span class="text-red-400">*</span></label>
                        <input wire:model="nama_dinas" type="text" class="w-full rounded-lg border border-slate-600 bg-slate-700/50 px-3 py-2 text-white text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="Dinas Sukamaju">
                        @error('nama_dinas') <p class="mt-1 text-xs text-red-400">{{ $message }}</p> @enderror
                    </div>
